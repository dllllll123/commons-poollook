...
+ For fast local regression or container builds, you can use the `quick` profile (e.g., `mvn -Pquick test` or `mvn -Pquick`), which performs compilation and unit testing while skipping heavy checks like javadoc, rat, and spotbugs.
